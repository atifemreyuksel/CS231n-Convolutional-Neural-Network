...
def svm_loss_vectorized(W, X, y, reg):
  ...
  num_class = W.shape[1]
  output = np.dot(X, W)
  margins = np.maximum(0, (output - np.expand_dims(output[np.arange(num_train), y], axis=1) + delta))
  ...
  active_y  = np.sum(margin_mask, axis=1)
  grad_mask = np.zeros(margins.shape)
  grad_mask[np.arange(num_train), y] = - (active_y).T
